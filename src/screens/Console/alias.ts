...
export function tablesCitees(sql: string): Map<string, string> {
  ...
  // `from` et `join` introduisent tous deux une table. `as` est optionnel en SQL, et le mockup
  // l'omet (`from orders o`).
  const motif = /\b(?:from|join)\s+([a-z_][\w$]*(?:\.[a-z_][\w$]*)?)\s*(?:as\s+)?([a-z_][\w$]*)?/gi
  for (const trouve of nu.matchAll(motif)) {
    ...
    // Un alias qui serait un mot-clé n'est pas un alias : `from orders where …` ferait de `where` un
    // alias, et proposerait les colonnes d'`orders` après `where.`.
    if (alias && !MOTS_RESERVES.has(alias.toLowerCase())) {
      par.set(alias.toLowerCase(), nom)
    }
  }
  return par
}
