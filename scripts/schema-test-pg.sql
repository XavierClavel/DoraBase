...
--
--   tables         dont une avec clé étrangère et contrainte CHECK, une aux deux formes
--                  d'identité, plus deux de tailles
...
-- Les deux formes d'identité de la norme SQL, que `bigserial` ne couvre pas : PostgreSQL ne les
-- range pas dans `pg_attrdef`, donc leur `default` est NULL. Sans cette table, le DDL de `14c`
-- pouvait perdre la clause `GENERATED … AS IDENTITY` sans qu'aucun test ne le voie — le rejeu
-- réussissait, et la copie n'avait plus d'auto-incrément.
create table introspection.identites (
  id bigint generated by default as identity primary key,
  rang integer generated always as identity,
  etiquette text not null
);
insert into introspection.identites (etiquette) values ('a'), ('b');
